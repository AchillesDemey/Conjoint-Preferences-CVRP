...
from PARAMETERS import PEAK_ROUTING, TAU, RHO, OMEGA
from models.RoutingModel import RoutingModel
from solvers.RoutingSolverGenetic import RoutingSolver
from solvers.RoutingSolverExhaustive import solve_exhaustive
...
def routing_test(train_from=130, train_to=165, test_to=200, peak_form='exp'):
    ...
    for problem_id in range(train_to, test_to):
        ...
        with open('./data/routing_path_eval_' + peak_form + '.data', 'wb') as output_file:
            pickle.dump(linear_test_data_path, output_file)
        with open('./data/routing_sol_eval_' + peak_form + '.data', 'wb') as output_file:
...
def read_routing_test_data(peak_form='exp'):
    with open('./data/routing_path_eval_' + peak_form + '.data', 'rb') as input_file:
        ltdp = pickle.load(input_file)
    with open('./data/routing_sol_eval_' + peak_form + '.data', 'rb') as input_file:
        ltds = pickle.load(input_file)
    return ltdp, ltds


# EXECUTE THE TEST
routing_test(TAU, RHO, OMEGA, PEAK_ROUTING)
